	...
	if (is_array($numbers) && @sizeof($numbers) != 0) {
		...
		foreach($numbers as $number) {
			if ($current_contact != '' && $number == $current_contact) {
				echo "<tr><td valign='top' class='contact row_style0 contact_selected' style='cursor: default;'>\n";
				$selected = true;
			}
			else {
				echo "<tr><td valign='top' class='contact row_style1' onclick=\"load_thread('".urlencode($number)."', '".$contact[$number]['contact_uuid']."');\">\n";
				$selected = false;
			}
			//contact image
